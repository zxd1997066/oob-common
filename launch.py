...
class MultiInstanceLauncher(Launcher):
    ...
    def launch(self, args):
        ...
        if args.core_list:  # user specify what cores will be used by params
            ...
            cores = [[int(x_) for x_ in x.split(",")] for x in args.core_list]
            args.ninstances = len(cores)
            args.ncore_per_instance = len(cores[0])
        else:
            ...
            if not args.multi_instance and args.ninstances == -1 and args.ncore_per_instance == -1:
                ...
                if args.ninstances * args.ncore_per_instance > len(cores):
                    logger.warning("Please make sure ninstances * ncore_per_instance <= total_cores. And there will be 1 instance not enough for ncore_per_instance")
                    # exit(-1)
        ...
        for i in range(args.ninstances):
            cmd = ["OMP_NUM_THREADS={}".format(len(cores[i]))]
            cur_process_cores = ""
...
def add_multi_instance_params(parser):
    ...
    group.add_argument("--core_list", nargs='*', help="Specify output for neural_compressor ckpt.")
    # group.add_argument("--core_list", default=None, type=list,
